...
const getItemGenre = (request, response) => {
   // const id = parseInt(request.params.id);
    ...
    pool.query('SELECT * FROM Catalog JOIN GenreType ON (Catalog.genretype = GenreType.ID) WHERE GenreType.genre ILike $1', [id], (error, results) => {
        response.status(200).json(results.rows);
    });
};
...
const renew = (request, response) => {
  ...
  const id = parseInt(request.params.id);
    ...
    pool.query('UPDATE TransOrder SET Duedate = $1 WHERE Patronlibid = $2', [duedate, id], (error, results) => {
        response.status(200).send(`Book renewed for Patron with id ${id}.`);
	});
};
...
const lookupActive = async (request, response) => {
    pool.query("SELECT Catalogid, Transorder.Patronlibid, Catalog.Title, Catalog.Author, Transorder.transdate::date From ITEM JOIN Catalog ON (item.catalogid = catalog.id) JOIN Cart ON(Cart.ItemID = Item.ID) JOIN TransOrder ON(TransOrder.ID = Cart.TransOrderID) WHERE Status = 'A'", (error, results) => {
        response.status(200).json(results.rows);
    });
};
...
const addItem = async (request, response) => {
    ...
    pool.query('Insert into catalog (ID, dewynum, itemtype, genretype, isbn, title, author, pubdate) Values ($8, $1,  $2, $3, $4, $5, $6, $7)', 
    [dewynum, itemtype, genretype, isbn, title, author, pubdate, ID], (error, results) => {
        //if (error) throw error;
